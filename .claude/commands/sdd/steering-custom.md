...
1. **Ask user** which custom steering document to create by presenting ALL available templates (both Project patterns and Integrations groups) in a **single question**. Do not ask for domain/topic separately or split into multiple questions.
...
8. **cross-repo.md** - Cross-repo contracts, ownership boundaries, and communication protocols for multi-repo products *(multi-repo — discovers and writes to all platform repos)*. *Produces persistent AI context — does not create specs.*

**Integrations** (written verbatim after displaying embedded setup guide):
9. **jira.md** - Jira Cloud sync: auto-creates Epics, Stories, and Subtasks at each SDD phase
10. **azure-devops.md** - Azure DevOps Boards sync: auto-creates Epics, User Stories, and Tasks at each SDD phase
